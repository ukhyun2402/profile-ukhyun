...
function wavify(wave_element, options) {
  if ("undefined" === typeof options) options = {}; //  Options
  //
  //
  ...
  var wave = wave_element,
      ...
      tweenMaxInstance = false; //  Allow new settings, avoid setting new container for logic purpose please :)
  //
  ...
  function updateColor(options) {
    if (_typeof(options.timing) === undefined) {
      options.timing = 1;
    }

    if (_typeof(options.color) === undefined) {
    ...
    tweenMaxInstance = TweenMax.to(wave, parseInt(options.timing), {
      ...
      onComplete: function onComplete() {
        if (_typeof(options.onComplete) !== undefined && {}.toString.call(options.onComplete) === "[object Function]") {
          options.onComplete();
        }
      }
    });
